...
-- GLOBALS: _G, ceil
-- GLOBALS: GetInventoryItemLink, GetContainerItemLink, GetInboxItemLink, GetAuctionItemLink, GetGuildBankItemLink
-- GLOBALS: GetItemInfo, GetItemInfoInstant, GetVoidItemInfo
-- GLOBALS: CursorHasItem, PickupInventoryItem, PickupContainerItem, PickupGuildBankItem, ClickVoidStorageSlot, TakeInboxItem

local bor, band, lshift = bit.bor, bit.band, bit.lshift
local EquipmentManager_UnpackLocation = EquipmentManager_UnpackLocation
-- location offsets
local ITEM_INVENTORY_BAG_BIT_OFFSET  = _G.ITEM_INVENTORY_BAG_BIT_OFFSET
local ITEM_INVENTORY_LOCATION_BAGS   = _G.ITEM_INVENTORY_LOCATION_BAGS
local ITEM_INVENTORY_LOCATION_BANK   = _G.ITEM_INVENTORY_LOCATION_BANK
local ITEM_INVENTORY_LOCATION_PLAYER = _G.ITEM_INVENTORY_LOCATION_PLAYER
local ITEM_INVENTORY_LOCATION_VAULT  = _G.ITEM_INVENTORY_LOCATION_VOIDSTORAGE
-- slot offsets
...
if not _G.EQUIPMENT_CONTAINER      then _G.EQUIPMENT_CONTAINER      = _G.REAGENTBANK_CONTAINER - 1 end
if not _G.GUILDBANK_CONTAINER      then _G.GUILDBANK_CONTAINER      = _G.EQUIPMENT_CONTAINER - 1 end
if not _G.VOIDSTORAGE_CONTAINER    then _G.VOIDSTORAGE_CONTAINER    = _G.GUILDBANK_CONTAINER - 1 end
if not _G.MAILATTACHMENT_CONTAINER then _G.MAILATTACHMENT_CONTAINER = _G.VOIDSTORAGE_CONTAINER - 1 end
if not _G.AUCTIONHOUSE_CONTAINER   then _G.AUCTIONHOUSE_CONTAINER   = _G.MAILATTACHMENT_CONTAINER - 1 end

-- existing location container identifiers
local EQUIPMENT_CONTAINER, BACKPACK_CONTAINER, BANK_CONTAINER, KEYRING_CONTAINER, REAGENTBANK_CONTAINER = _G.EQUIPMENT_CONTAINER, _G.BACKPACK_CONTAINER, _G.BANK_CONTAINER, _G.KEYRING_CONTAINER, _G.REAGENTBANK_CONTAINER
-- custom created container variables
local GUILDBANK_CONTAINER, VOIDSTORAGE_CONTAINER, MAILATTACHMENT_CONTAINER, AUCTIONHOUSE_CONTAINER = _G.GUILDBANK_CONTAINER, _G.VOIDSTORAGE_CONTAINER, _G.MAILATTACHMENT_CONTAINER, _G.AUCTIONHOUSE_CONTAINER
...
function lib:GetLocation(container, slot, index)
	...
	elseif container == GUILDBANK_CONTAINER then
		if index then
			container = slot
			slot      = index
		else
