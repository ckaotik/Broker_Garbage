...
UpdateAuctionAddonList = function(panel)
	...
	for _, displayType in ipairs(displayTypes) do
		...
		for i, addonKey in ipairs(auctionAddonOrder) do
			auctionAddon = Broker_Garbage.GetPriceHandler(addonKey)
			...
			if i == #(auctionAddonOrder) then addonLine.moveDown:Hide()
			else addonLine.moveDown:Show() end
			addonLine:Show()
		end

		for i = #(auctionAddonOrder) + 1, #frames[displayType] do
			frames[i]:Hide()
		end
	end
end
